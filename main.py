...
from models import Job, JobDescription, Candidate
...
def get_key_traits(job_description: JobDescription):
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role": "system", "content": key_traits_prompt}, {"role": "user", "content": job_description.description}],
...
async def create_candidate(job_id: str, candidate: Candidate):
    ...
    graph_result = await run_search(job_data["job_description"], candidate_data["context"], candidate_data["name"], job_data["key_traits"], 10)
